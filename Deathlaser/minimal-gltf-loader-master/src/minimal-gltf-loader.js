...
var BoundingBox = MinimalGLTFLoader.BoundingBox = function (min, max, isClone) {
    // this.min = min;
    // this.max = max;
    min = min || glMatrix.vec3.fromValues(Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY);
    max = max || glMatrix.vec3.fromValues(Number.NEGATIVE_INFINITY, Number.NEGATIVE_INFINITY, Number.NEGATIVE_INFINITY);

    if (isClone === undefined || isClone === true) {
        this.min = glMatrix.vec3.clone(min);
        this.max = glMatrix.vec3.clone(max);
    } else {
        this.min = min;
        this.max = max;
    }
...
BoundingBox.prototype.updateBoundingBox = function (bbox) {
    glMatrix.vec3.min(this.min, this.min, bbox.min);
    glMatrix.vec3.max(this.max, this.max, bbox.max);
};
...
BoundingBox.getAABBFromOBB = (function() {
    let transformRight = glMatrix.vec3.create();
    let transformUp = glMatrix.vec3.create();
    let transformBackward = glMatrix.vec3.create();

    let tmpVec3a = glMatrix.vec3.create();
    let tmpVec3b = glMatrix.vec3.create();

    return (function (obb, matrix) {
        glMatrix.vec3.set(transformRight, matrix[0], matrix[1], matrix[2]);
        glMatrix.vec3.set(transformUp, matrix[4], matrix[5], matrix[6]);
        glMatrix.vec3.set(transformBackward, matrix[8], matrix[9], matrix[10]);

        let min = glMatrix.vec3.fromValues(matrix[12], matrix[13], matrix[14]);  // init with matrix translation
        let max = glMatrix.vec3.clone(min);

        glMatrix.vec3.scale(tmpVec3a, transformRight, obb.min[0]);
        glMatrix.vec3.scale(tmpVec3b, transformRight, obb.max[0]);
        glMatrix.vec3.min(transformRight, tmpVec3a, tmpVec3b);
        glMatrix.vec3.add(min, min, transformRight);
        glMatrix.vec3.max(transformRight, tmpVec3a, tmpVec3b);
        glMatrix.vec3.add(max, max, transformRight);

        glMatrix.vec3.scale(tmpVec3a, transformUp, obb.min[1]);
        glMatrix.vec3.scale(tmpVec3b, transformUp, obb.max[1]);
        glMatrix.vec3.min(transformUp, tmpVec3a, tmpVec3b);
        glMatrix.vec3.add(min, min, transformUp);
        glMatrix.vec3.max(transformUp, tmpVec3a, tmpVec3b);
        glMatrix.vec3.add(max, max, transformUp);

        glMatrix.vec3.scale(tmpVec3a, transformBackward, obb.min[2]);glMatrix.
        glMatrix.vec3.scale(tmpVec3b, transformBackward, obb.max[2]);
        glMatrix.vec3.min(transformBackward, tmpVec3a, tmpVec3b);
        glMatrix.vec3.add(min, min, transformBackward);
        glMatrix.vec3.max(transformBackward, tmpVec3a, tmpVec3b);
        glMatrix.vec3.add(max, max, transformBackward);

        let bbox = new BoundingBox(min, max, false);
...
let Accessor = MinimalGLTFLoader.Accessor = function (a, bufferViewObject) {
...
let BufferView = MinimalGLTFLoader.BufferView = function(bf, bufferData) {
...
let Camera = MinimalGLTFLoader.Camera = function(c) {
...
let Node = MinimalGLTFLoader.Node = function (n, nodeID) {
    ...
    if (n.extensions !== undefined) {
        if (n.extensions.gl_avatar !== undefined && curLoader.enableGLAvatar === true) {
            let linkedSkinID = curLoader.skeletonGltf.json.extensions.gl_avatar.skins[ n.extensions.gl_avatar.skin.name ];
            let linkedSkin = curLoader.skeletonGltf.skins[linkedSkinID];
...
let Mesh = MinimalGLTFLoader.Mesh = function (m, meshID) {
    this.meshID = meshID;
...
let Primitive = MinimalGLTFLoader.Primitive = function (gltf, p) {
    ...
    if (p.extensions !== undefined) {
        if (p.extensions.gl_avatar !== undefined && curLoader.enableGLAvatar === true) {
            if (p.extensions.gl_avglMatrix.atar.attributes) {
    ...
    if (this.attributes.POSITION !== undefined) {
        ...
        if (accessor.max) {
            ...
            if (accessor.type === 'VEC3') {
                this.boundingBox = new BoundingBox(
                    glMatrix.vec3.fromValues(accessor.min[0], accessor.min[1], accessor.min[2]),
                    glMatrix.vec3.fromValues(accessor.max[0], accessor.max[1], accessor.max[2]),
                    false
                );
                this.boundingBox.calculateTransform();
...
let Texture = MinimalGLTFLoader.Texture = function (t) {
...
let Sampler = MinimalGLTFLoader.Sampler = function (s) {
    this.name = s.name !== undefined ? s.name : null;
...
let TextureInfo = MinimalGLTFLoader.TextureInfo = function (json) {
...
let PbrMetallicRoughness = MinimalGLTFLoader.PbrMetallicRoughness = function (json) {
...
let NormalTextureInfo = MinimalGLTFLoader.NormalTextureInfo = function (json) {
    this.index = json.index;
    this.texCoord = json.texCoord !== undefined ? json.texCoord : 0 ;
...
let OcclusionTextureInfo = MinimalGLTFLoader.OcclusionTextureInfo = function (json) {
    this.index = json.index;
    this.texCoord = json.texCoord !== undefined ? json.texCoord : 0 ;
...
let Material = MinimalGLTFLoader.Material = function (m) {
    this.name = m.name !== undefined ? m.name : null;
...
let Skin = MinimalGLTFLoader.Skin = function (gltf, s, skinID) {
    this.name = s.name !== undefined ? s.name : null;
    this.skinID = skinID;
...
let SkinLink = MinimalGLTFLoader.SkinLink = function (gltf, linkedSkin, inverseBindMatricesAccessorID) {
    this.isLink = true;
    ...
    if (this.inverseBindMatrices) {
        ...
        for (let i = 0, len = this.inverseBindMatricesData.length; i < len; i += 16) {
            this.inverseBindMatrix.push(mat4.fromValues(
                this.inverseBindMatricesData[i],
                this.inverseBindMatricesData[i + 1],
                this.inverseBindMatricesData[i + 2],
...
let Target = MinimalGLTFLoader.Target = function (t) {
...
let Channel = MinimalGLTFLoader.Channel = function (c, animation) {
...
let AnimationSampler = MinimalGLTFLoader.AnimationSampler = function (gltf, s) {
    ...
    this.curIdx = 0;
    // this.curValue = 0;
    this.curValue = glMatrix.vec4.create();
    this.endT = this.inputTypedArray[this.inputTypedArray.length - 1];
    this.inputMax = this.endT - this.inputTypedArray[0];
};

let animationOutputValueVec4a = glMatrix.vec4.create();
let animationOutputValueVec4b = glMatrix.vec4.create();
...
AnimationSampler.prototype.getValue = function (t) {
    ...
    var v4lerp = count === 4 ? glMatrix.quat.slerp: glMatrix.vec4.lerp;
...
glTFLoader.prototype._postprocess = function () {
    ...
    for(i = 0, leni = nodeMatrix.length; i < leni; i++) {
        nodeMatrix[i] = glMatrix.mat4.create();
    }
    ...
    function execUpdateTransform(n, parent) {
        ...
        if (parent !== null) {
            glMatrix.mat4.mul(tmpMat4, nodeMatrix[parent.nodeID], n.matrix);
        } else {
            glMatrix.mat4.copy(tmpMat4, n.matrix);
        }
    }
        ...
        if (n.mesh) {
            ...
            if (mesh.boundingBox) {
                ...
                if (n.children.length === 0) {
                    // n.bvh = n.aabb;
                    glMatrix.vec3.copy(n.bvh.min, n.aabb.min);
                    glMatrix.vec3.copy(n.bvh.max, n.aabb.max);
                }
            }
        }

        glMatrix.vec3.min(parentBVH.min, parentBVH.min, n.bvh.min);
        glMatrix.vec3.max(parentBVH.max, parentBVH.max, n.bvh.max);
    }
